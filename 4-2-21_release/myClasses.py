...
class Airplane(object):
    ...
    def generateFaceDicts(self, mode):
        self.allFaces = {}
        self.renderedFaces = {}
        self.allVertices = []
        for part in self.getPartsInPosition():
            vertices = getVertices(mode, part[1:-1])
            self.allVertices.append(vertices)
            getFaces(mode, part, self.allFaces, vertices)
            renderFaces(mode, part, self.renderedFaces, vertices)
      

    #Draws all the parts that make up the plane
    def draw(self, mode, canvas):
        drawShapes(mode, canvas, self.renderedFaces, self.allFaces, self.partNames)

    def drawShadow(self, mode, canvas):
        if mode.app.inputBoxVars["accurateShadow"]: 
            drawAccurateShadow(mode, canvas, self.allVertices)
        else:
            drawShadow(mode, canvas, mode.app.airplaneSize * .75, mode.app.airplane.center, True)
...
class Quad(Airplane):
    ...
    def performControllerInputs(self, mode):
       ...
       # controlAuthority = self.getControlAuthority(mode, throttle)
        scalingFactor = 6
        pitch = mode.app.channelAssignment["pitch"].getValue(mode.app)
        self.pitch(scalingFactor * mode.app.inputBoxVars["pitchRate"] * pitch / mode.framerate)
        yaw = mode.app.channelAssignment["yaw"].getValue(mode.app)
        self.yaw(-scalingFactor * mode.app.inputBoxVars["yawRate"] * yaw / mode.framerate)
        roll = mode.app.channelAssignment["roll"].getValue(mode.app)
        self.roll(scalingFactor * mode.app.inputBoxVars["rollRate"] * roll / mode.framerate)
...
class Cloud(object):
    ...
    def generateCloudPart(self):
        ...
        #In previous versions there was a vertical and horizontal radius to allow ellipses, but with the addition of a rotating camera,
        #The cloud parts must be "spheres"
        #The code has not been modified to remove support for the two different radii in case I make the state of the clouds dependent on the model type
        tempR = random.randint(minR, maxR)
        partR = (tempR, tempR)
...
class Bush(object):
    ...
    def __init__(self, mode):
        minX, maxX = -mode.app.inputBoxVars["mapSize"] * 1.1, mode.app.inputBoxVars["mapSize"] * 1.1
        minY, maxY = -mode.app.inputBoxVars["mapSize"] * 1.1, mode.app.inputBoxVars["mapSize"] * 1.1
        self.r = random.uniform(1, 3)
        self.position = None
        while self.position == None:
            pos = (random.randint(int(minX), int(maxX)), random.randint(int(minY), int(maxY)), self.r * .8)
...
class Tree(object):
    ...
    def __init__(self, mode):
        ...
        self.position = None
        while self.position == None:
            pos = (random.randint(int(minX), int(maxX)), random.randint(int(minY), int(maxY)), random.randint(int(1.5 * self.r), int(2.5 * self.r)))
            if distance(pos, (0, 0, 0)) > 50:
                self.position = pos
        self.color = [25, 120, 25]
    ...
    def draw(self, mode, canvas):
        if mode.observer[2] < self.position[2] - self.r:
            drawCircle(mode, canvas, self, self.color, 0)
    ...
    def drawTrunk(self, mode, canvas):
        ...
        for i in range(pointCount):
            ...
            x, y = getXandY(mode, (self.position[0] + self.r/5 * math.cos(angle), self.position[1] + self.r/5 * math.sin(angle), 0))
            if x != None:
                points.append((x, y))
        canvas.create_polygon(points, fill = scaleColorByTimeOfDay(mode, color, 1/3), width = 0)
        
    def drawTrunkShadow(self, mode, canvas):
        shadowX1, shadowY1 = (self.position[0], self.position[1])
        shadowX2, shadowY2 = getShadowPosition(mode, self.position)
        horVector = vectorSum((shadowX2, shadowY2, 0), 1, (shadowX1, shadowY1, 0), -1)
        perpVector = makeUnitVector(crossProduct(horVector, (0, 0, 1)))
        p1 = addVectorToPoint((shadowX1, shadowY1, 0), perpVector, self.r/5)
        p2 = addVectorToPoint((shadowX2, shadowY2, 0), perpVector, self.r/5)
        p3 = addVectorToPoint((shadowX2, shadowY2, 0), perpVector, -self.r/5)
        p4 = addVectorToPoint((shadowX1, shadowY1, 0), perpVector, -self.r/5)
        poly = []
...
#Draws all the trees and bushes in a provided list
def drawAllTreesAndBushes(objects, mode, canvas):
    for currObject in objects:
        currObject.draw(mode, canvas)

#Creates a list of all the trees and bushes and sorts them based on distance to the observer.
#Splits the list into two lists at the distance from the observer to the aircraft
...
#Finds the radius on the Tkinter canvas that a sphere with a certain actual radius should be drawn as
#Also returns the center of the circle in case it is a sphere being drawn 
...
#Creates a runway class that can be used to make runways
class Runway(object):

    #Initializes the runway values. Currently hardcoded in because I only need one runway
    def __init__(self):
        self.yCount = 6
        self.xCount = 30
        self.ySize = 6
    ...
    #Draws the runway. Creates a polygon in the shape of a rectangle but with numerous points per side. This allows the runway to be partially rendered
    #even if some part of the runway is off the screen.
    def draw(self, mode, canvas):
        side1 = [(self.startPos[0] + i*self.dx, self.startPos[1], 0) for i in range(self.xCount + 1)]
        side2 = [(self.startPos[0] + self.xSize, self.startPos[1] + i*self.dy, 0) for i in range(self.yCount + 1)]
        side3 = [(self.startPos[0] + self.xSize - i*self.dx, self.startPos[1] + self.ySize, 0) for i in range(self.xCount + 1)]
        side4 = [(self.startPos[0], self.startPos[1] + self.ySize - i*self.dy, 0) for i in range(self.yCount + 1)]
        pointList = side1 + side2 + side3 + side4
        poly = []
        for point in pointList:
            (x,y) = getXandY(mode, point)
            if x != None:
                poly.append((x,y))
        #Draws the runway as long as there are 3 or more points.
        if len(poly) >= 3:
            drawPolyFace(mode, canvas, (poly, self.color), True, 0)
