...
def startRollTraining(mode):
    ...
    mode.app.airplane.d2 = (0, 0, 1)
    mode.app.airplane.d1 = (1, 0, 0)
    mode.app.airplane.center = (5, 5, 1)
    mode.trainingText = """\
...
def pitchTraining(mode):
    ...
    mode.app.airplane.performControllerInputs(mode)
    lookAtPlane(mode)
    if dotProduct(mode.app.airplane.d1, (0, 0, 1)) > .95:
        trainingCompleted(mode)
...
def yawTraining(mode):
    ...
    mode.app.airplane.performControllerInputs(mode)
    lookAtPlane(mode)
    if dotProduct(mode.app.airplane.d1, (-1, 0, 0)) > .95:
        trainingCompleted(mode)
...
def startThrottleTraining(mode):
    ...
    mode.followPlaneMode = False
    mode.app.airplane.d2 = (-.5 ** .5, 0, .5 ** .5)
    mode.app.airplane.d1 = (.5 ** .5, 0, .5 ** .5)
    mode.trainingText = """\
...
def throttleTraining(mode):
    ...
    mode.app.airplane.center = addVectorToPoint(mode.app.airplane.center, mode.app.airplane.d1, throttle)
    lookAtPlane(mode)
    if throttle >= 4.5:
        trainingCompleted(mode)
...
def turningTraining(mode):
    ...
    mode.app.airplane.performControllerInputs(mode)
    lookAtPlane(mode)
    if abs(dotProduct(mode.app.airplane.d2, (0, -1, 0))) > .95:
        mode.rollActive = False
    if abs(mode.app.airplane.d1[1]) > .9:
        trainingCompleted(mode)
...
def startTakeOffTraining(mode):
    ...
    mode.followPlaneMode = False
    mode.app.airplane.center = (-35, 5, 0)
    mode.app.airplane.d2 = (0, 0, 1)
    mode.app.airplane.d1 = (1, 0, 0)
    mode.app.airplane.velocity = 0
    mode.trainingText = """\
...
def takeOffTraining(mode):
    ...
    lookAtPlane(mode)
    if mode.app.airplane.center[2] > 15:
        trainingCompleted(mode)

def startLandingTraining(mode):
    mode.trainingSelection = "landingTraining"
    mode.followPlaneMode = False
    mode.app.airplane.center = (-60, 5, 20)
    mode.app.airplane.d2 = (1/2, 0, 3**.5/2)
    mode.app.airplane.d1 = (3**.5 / 2, 0, -1/2)
    mode.app.airplane.velocity = 15
...
def landingTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if mode.timeCompleted == None and mode.app.airplane.justTouchedGround and not mode.crashed:
...
def startAirTurningTraining(mode):
    ...
    mode.followPlaneMode = False
    mode.app.airplane.center = (-40, 20, 15)
    mode.app.airplane.d2 = (0, 0, 1)
    mode.app.airplane.d1 = (1, 0, 0)
    mode.app.airplane.velocity = 15
    mode.trainingText = """\
...
def airTurningTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if dotProduct(mode.app.airplane.d1, (-1, 0, 0)) > .9 and dotProduct(mode.app.airplane.d2, (0, 0, 1)) > .9:
        trainingCompleted(mode)

def drawHeadingVector(mode, canvas):
    goalVector = (-1, 0, 0)
    planePos = getXandY(mode, mode.app.airplane.center)
    vectorTipPos = getXandY(mode, addVectorToPoint(mode.app.airplane.center, goalVector, 5))
...
def startLanding2Training(mode):
    ...
    mode.followPlaneMode = False
    mode.app.airplane.center = (-30, 10, 10)
    mode.app.airplane.d1 = (1, 0, 0)
    mode.app.airplane.d2 = (0, 0, 1)
    mode.app.airplane.pitch(-70)
    mode.app.airplane.roll(-40)
    mode.app.airplane.yaw(15)
    mode.app.airplane.velocity = 15
    mode.timeCompleted = None
    mode.trainingText = """\
...
def startProlongedTraining(mode):
    ...
    mode.followPlaneMode = False
    mode.app.airplane.center = (-20, 10, 0)
    mode.app.airplane.d2 = (0, 0, 1)
    mode.app.airplane.d1 = (1, 0, 0)
    mode.app.airplane.velocity = 0
...
def prolongedTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if mode.app.airplane.justTouchedGround:
        mode.timerEnd = time.time() + 20
...
def invertedTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if dotProduct(mode.app.airplane.d2, (0, 0, 1)) > 0:
        mode.timerEnd = time.time() + 10
    if time.time() > mode.timerEnd:
        trainingCompleted(mode)
...
def knifeTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if magnitude(crossProduct(mode.app.airplane.d2, (0, 0, 1))) < .75 or mode.app.airplane.center[2] > 10:
        mode.timerEnd = time.time() + 10
    if time.time() > mode.timerEnd:
        trainingCompleted(mode)
...
def lowFlyingTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if mode.app.airplane.center[2] > 5 or mode.app.airplane.justTouchedGround:
        mode.timerEnd = time.time() + 15
    if time.time() > mode.timerEnd:
        trainingCompleted(mode)
...
def verticalTraining(mode):
    ...
    mode.app.airplane.performPhysics(mode)
    lookAtPlane(mode)
    if dotProduct(mode.app.airplane.d1, (0, 0, 1)) < .8:
        mode.timerEnd = time.time() + 10
    if time.time() > mode.timerEnd:
        trainingCompleted(mode)
