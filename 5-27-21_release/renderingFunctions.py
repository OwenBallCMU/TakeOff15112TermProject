...
def createGround(mode):
    ...
    mode.grassRange = mode.app.inputBoxVars["mapSize"] * 1.25
    mode.floor = {}
...
def getFaces(mode, shape, allFaces, vertices):
    name = shape[0]
    color = shape[-1]
    for i in range(6):
...
def renderFaces(mode, shape, renderedFaces, vertices):
    name = shape[0]
    color = shape[-1]
...
def drawGrassSquare(mode, canvas, row, col):
    points = getGrassCorners(mode, row, col)
    color = mode.floor[(row, col)]
    drawGroundRectangle(mode, canvas, points, color, 4)
...
def drawGroundRectangle(mode, canvas, points, color, count):
    poly = []
    renderedPoints = [getXandY(mode, point) for point in points]
    for i in range(len(points)):
        p1 = renderedPoints[i-1]
        p2 = renderedPoints[i]
        if p1[0] != None:
            poly.append(p1)
        if (p1[0] == None) ^ (p2[0] == None):
            if p1[0] != None:
                primary = points[i-1]
                secondary = points[i]
            else:
                primary = points[i]
                secondary = points[i-1]
            for j in range(count):
                checkPoint = averagePoints(primary, secondary)
                if getXandY(mode, checkPoint)[0] == None:
                    secondary = checkPoint
                else:
                    primary = checkPoint
            poly.append(getXandY(mode, primary))
    if poly != []:
        #print(poly)
...
def drawShadow(mode, canvas, r, objectPosition, stippling = False):
    ...
    if (isinstance(mode.app.airplane.velocity, list) and mode.followPlaneMode):
        drawFPVShadow(mode, canvas, r, shadowX, shadowY, stippling)
        return
    ...
    if stippling:
        ...
        for i in range(pointCount):
            ...
            points.append((cx + horR * math.cos(angle), cy + vertR * math.sin(angle)))
        canvas.create_polygon(points, fill = color, stipple = "gray50", width = 0)
    else:
        color = scaleColorByTimeOfDay(mode, (30, 80, 30), 1/3)
        canvas.create_oval(cx - horR, cy - vertR, cx + horR, cy + vertR, fill = color, width = 0)

def drawFPVShadow(mode, canvas, r, shadowX, shadowY, stippling):
    pointCount = 20
    points = []
    for i in range(pointCount):
        angle = 2*math.pi * i / pointCount
        x, y = getXandY(mode, (shadowX + r * math.cos(angle), shadowY + r * math.sin(angle), 0))
        if x != None:
            points.append((x, y))
...
def drawPolyShadow(mode, canvas, poly):
    color = rgbString((0, 0, 0))
    canvas.create_polygon(poly, fill = color, stipple = "gray50", width = 0)
 
def drawAccurateShadow(mode, canvas, allVertices):
    allFaces = []
    for vertices in allVertices:
        allFaces.append(getShadowFaces(mode, vertices))
    """
    if mode.app.inputBoxVars["useShapely"]:
        for faces in allFaces:
            facePolys = [Polygon(face) for face in faces]
            facePoly = unary_union(facePolys)
            if type(facePoly) == Polygon:
                coords = list(facePoly.exterior.coords)
                drawPolyShadow(mode, canvas, coords)
    """
    for faces in allFaces:
        for face in faces:
            drawPolyShadow(mode, canvas, face)

#Generates all the faces of an object and adds them to a dictionary
def getShadowFaces(mode, vertices):
    faces = []
    renderedVertices = {}
    for point in vertices:
        shadowPos = getShadowPosition(mode, vertices[point])
        renderedVertices[point] = getXandY(mode, (shadowPos[0], shadowPos[1], 0))
    for i in range(6):
        face = makeShadowFace(mode, mode.faceSetups[i], renderedVertices)
        if len(face) == 4:
            faces.append(face)
    return faces
...
#Creates a single face of the object and adds it to a dictionary
def makeShadowFace(mode, faceVertices, vertices):
    face = []
    for vertex in faceVertices:
        face.append(vertices[f"c{vertex}"])
    newFace = []
    for point in face:
        if point != (None, None):
