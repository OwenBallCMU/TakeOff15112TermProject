...
class Airplane(object):
    ...
    def getPartsInPosition(self):
        ...
        for part in self.parts:
            ...
            partsInPosition.append((name, partCenter, self.d1, self.d2, length, height, width, color))
...
class Cloud(object):
    ...
    def generateCloudParts(self):
        numOfParts = random.randint(2, 20)
    ...
    def drawAll(bushes, mode, canvas, inFront):
        for bush in bushes:
            if not distance(bush.position, mode.observer) > mode.app.inputBoxVars["viewRange"]:
                isInFront = distance(mode.observer, bush.position) < distance(mode.observer, mode.airplane.center)
...
class Balloon(object):
    ...
    def __lt__(self, other):
        return distance(self.observerPosCopy, self.position) < distance(self.observerPosCopy, other.position)
    ...
    def drawAll(balloons, mode, canvas, inFront):
        for balloon in reversed(sorted(balloons)):
            if not distance(balloon.position, mode.observer) > mode.app.inputBoxVars["viewRange"]:
                isInFront = distance(mode.observer, balloon.position) < distance(mode.observer, mode.airplane.center)
                if isInFront == inFront:
    ...
    def touchingPlane(self, mode):
        ...
        if distance(self.position, mode.airplane.center) < 3 * self.r:
            #Main collision check
            for corner in mode.airplane.getHitboxCorners(mode).values():
...
def drawCircle(mode, canvas, circle, color, outline = 1):
    ...
    p0 = getXandY(mode, circle.position)
    if p0[0] == None: return
    tempVector = crossProduct(mode.viewDirection, (0, 0, -1))
    if magnitude(tempVector) == 0:
        rVector = (1, 0, 0)
    else:
        rVector = makeUnitVector(tempVector)
    p1 = getXandY(mode, addVectorToPoint(circle.position, rVector, circle.r))
    if p1[0] == None: return
    circleR = ((p1[1] - p0[1])**2 + (p1[0] - p0[0])**2)**.5
...
class ControllerChannel(object):
    ...
    #CITATION: Slider style inspired by: https://www.liftoff-game.com/
        ...
        for readout in self.dataReadouts:
            ...
            if mode.app.inputBoxVars[readout]:
                if readout == "velocityReading":
                    text += f"Velocity: {round(mode.airplane.velocity, 1)} m/s\n"
                elif readout == "altitudeReading":
                    text += f"Altitude: {round(mode.airplane.center[2], 1)} m\n"
                elif readout == "attitudeReading":
                    attitude = math.asin(dotProduct(mode.airplane.d1, (0, 0, 1))) * 180 / math.pi
                    text += f"Attitude: {round(attitude, 1)} degrees\n"
                elif readout == "distanceReading":
                    text += f"Distance: {round(distance(mode.airplane.center, (0, 0, 0)), 1)} m\n"
