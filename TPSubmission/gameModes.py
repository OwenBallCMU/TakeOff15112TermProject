...
def initializeGame(mode):
    ...
    mode.bushes = [Bush() for i in range(mode.app.inputBoxVars["bushCount"])]
    mode.crashed = False
    mode.followPlaneMode = False
    createGround(mode)
    mode.airplane = createAirplane(mode)
    lookAtPlane(mode)
...
def updateFramerate(mode):
    ...
    #Math for exponential moving average based off content in my 18100 class
    filterVal = .75
    mode.framerate = filterVal * mode.framerate + (1 - filterVal) * actualFramerate
...
class GameMode(Mode):
    ...
    def resetPlane(mode):
        mode.airplane.velocity, mode.airplane.center = 0, (0, 5, 0)
        mode.airplane.d1, mode.airplane.d2 = (1, 0, 0), (0, 0, 1)
        mode.prev = time.time()
        mode.crashed = False
    ...
    def timerFired(mode):
        ...
        if mode.followPlaneMode: followPlane(mode)
        else: gotoOrigin(mode)
        ...
        Bush.touchingABush(mode.bushes, mode.airplane.center, mode)
        lookAtPlane(mode)
        ...
        if distance(mode.airplane.center, (0, 0, 0)) >= 1750:
            mode.resetPlane()
    ...
    def redrawAll(mode, canvas):
        ...
        if not mode.crashed:
            ...
            mode.airplane.draw(mode, canvas)
        ...
        if distance(mode.airplane.center, (0, 0, 0)) >= 1500:
